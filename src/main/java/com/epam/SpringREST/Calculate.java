...
// wsgen -keep -cp . com.epam.ws.Calculate
...
public class Calculate {
     ...
     */
    public double calculate(String str) throws DivisionByZero {
        str = check(str);
        double result;
        i_str=0;
        String Flag = "number";
            ...
            if (Flag.equals("number")) {          // значит, число
                ...
                if (!operators.empty()) {
                    if (operators.peek() == '*') multiplyNum(numbers.pop(), numbers.pop());
                            ...
                            divideNum(numbers.pop(), numbers.pop());
     ...
     * @param y
     */
    public void divideNum(double x, double y) throws DivisionByZero {
        if (x==0) {
            throw new DivisionByZero("ERROR : деление на ноль");
        }
